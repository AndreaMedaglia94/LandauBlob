...
> **Implementation note:** `Functions/collisions_numba.py` and
> `Functions/fields.py` were rewritten for speed (cell-sorted particle
> arrays instead of linked-list cell lists, fused Step III, a fast path for
> `gam=-2`, parallel deposition, and a few other implementation-only
> changes) while keeping the exact same method and public API. The
> original files are preserved in `Functions_reference/`. Run
> `python test_regression.py` once after installing requirements to
> confirm the optimized code reproduces the original on your machine, and
> `python benchmark.py` to measure the actual speedup. Details and the
> validation methodology are in `OPTIMIZATION_NOTES.md`.
...
`BASE_KWARGS` block for changing the run. After a run finishes, it now also
prints a rough time breakdown (collisions / deposit+fields / push) so you can
see where time is actually going for your problem size.
...
implementation-only tuning knob (see `OPTIMIZATION_NOTES.md`): the particle
re-sort that precedes each Steps I-III sweep is parallelised over
`sort_oversample * numba.get_num_threads()` chunks. Raise it if profiling
shows the sort itself is a bottleneck on a many-core machine.
