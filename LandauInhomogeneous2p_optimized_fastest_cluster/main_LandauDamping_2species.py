...
BASE_KWARGS = dict(
    ...
    dist_save_interval=5,     # cadence for the .mat-saved f(x,v) marginal snapshots
    save_results=True,
...
# Figures
MAKE_PLOTS = True
PLOT_MARGINAL_TIMES = [0.0, 5.0, 10.0, 15.0, 20.0, 25.0, 30.0, 35.0, 40.0, 45.0, 50.0]  # physical times for the
...
def save_figures(elog, spelog, plot_marginals, par):
    ...
    plt.legend()
    fig_path = os.path.join("Figures", f"{par.output_prefix}_all_energies.png")
    ...
    plt.legend()
    fig_path = os.path.join("Figures", f"{par.output_prefix}_electric_energy.png")
    plt.savefig(fig_path, dpi=140, bbox_inches="tight")
    plt.close()
    print(f"Saved plot to {fig_path}")
