...
class Params:
    ...
    # ------------------------------------------------------------------
    # Implementation-only performance tuning (does not change the method).
    # Before every Steps I-III sweep, particles are physically re-sorted
    # into cell order (Functions/cell_sort.py) for cache-friendly
    # neighbour access; that sort is itself parallelised over
    # `sort_oversample * numba.get_num_threads()` chunks. 1 (default) uses
    # exactly one chunk per available thread; raise it (e.g. 2-4) on
    # many-core machines if profiling shows the sort phase itself is a
    # bottleneck relative to the pairwise sums -- at the cost of a
    # temporary (nchunks x ncells) int64 histogram buffer.
    # ------------------------------------------------------------------
    sort_oversample: int = 1
    ...
    # record_interval:    physical-time spacing for the History diagnostics
    #                      (temperature, momentum, current, entropy, ...).
    # dist_save_interval: physical-time spacing for saving the reconstructed
    #                      velocity marginals f_s(x, v1), f_s(x, v2) for both
    #                      species (see Functions/reconstruct.py). Independent
    #                      of record_interval since these snapshots are much
    #                      heavier than a handful of scalars.
    # save_results:        master toggle for writing the output .mat file.
    # save_distributions:  toggle for including the marginal snapshots in it
    #                      (they can be the bulk of the file size).
    # Figures are never written by this driver -- plotting is left to
    # MATLAB post-processing of the saved .mat file.
    # ------------------------------------------------------------------
    record_interval: float = 0.1
    dist_save_interval: float = 0.5
    ...
    def __post_init__(self):
        ...
        if self.dist_save_interval <= 0.0:
            raise ValueError("dist_save_interval must be positive")

        if self.dist_Nv1_v1 is None:
            self.dist_Nv1_v1 = self.Nv1_v1
        if self.dist_Nv1_v2 is None:
            self.dist_Nv1_v2 = self.Nv1_v2
        if self.dist_Nv2_v1 is None:
            self.dist_Nv2_v1 = self.Nv2_v1
        if self.dist_Nv2_v2 is None:
            self.dist_Nv2_v2 = self.Nv2_v2
        if min(self.dist_Nv1_v1, self.dist_Nv1_v2, self.dist_Nv2_v1, self.dist_Nv2_v2) <= 0:
            raise ValueError("dist_Nv1_v1/dist_Nv1_v2/dist_Nv2_v1/dist_Nv2_v2 must be positive")

        if self.auto_Lv2_equal_m_epsi:
            # Corollary 2.4 (Carrillo, Hu, Van Fleet 2024): keep m_i*eps_i
            # constant across species via eps_i ~ h_i^1.98, i.e.
            # Lv2 = Lv1 * (m1/m2)^(1/1.98).
            self.Lv2_v1 = self.Lv1_v1 * (self.m1 / self.m2) ** (1.0 / 1.98)
            self.Lv2_v2 = self.Lv1_v2 * (self.m1 / self.m2) ** (1.0 / 1.98)
...
# Derived (read-only) properties worth carrying into the saved .mat struct
# alongside the dataclass fields themselves.
_PARAMS_EXTRA_PROPERTIES = (
    "ntot", "particles_per_x_cell_1", "particles_per_x_cell_2",
    "q_over_m1", "q_over_m2", "current_factor1", "current_factor2",
    "collision_active", "gamma_landau_collisionless",
)


# MATLAB's .mat v5 struct field names are capped at 31 characters;
# rename the (currently one) field that overflows that limit.
_MATLAB_FIELD_RENAMES = {
    "enforce_zero_species_mean_velocity": "enforce_zero_species_mean_v",
}


def params_to_matlab_dict(par: "Params") -> dict:
    """Flatten a Params object (dataclass fields + a few derived properties)
    into a plain dict of scalars/strings/arrays suitable for
    scipy.io.savemat, which cannot represent Python None and caps struct
    field names at 31 characters.  None fields (e.g. random_seed=None)
    become an empty array; overlong names are renamed/truncated;
    everything else is passed through unchanged.
    """
    flat = asdict(par)
    for name in _PARAMS_EXTRA_PROPERTIES:
        flat[name] = getattr(par, name)
    out = {}
    for key, value in flat.items():
        mkey = _MATLAB_FIELD_RENAMES.get(key, key)
        if len(mkey) > 31:
            mkey = mkey[:31]
        out[mkey] = np.array([]) if value is None else value
    return out
