...
# -----------------------------------------------------------------------------
# Fast paths for the default compact linear (hat) spline.  The generic spline
# routine supports orders 1, 2 and 3, but carrying the order branch in the inner
# loops is expensive.  These helpers are intentionally tiny and are inlined by
# numba in the hot kernels below.
# -----------------------------------------------------------------------------
...
@njit(cache=True)
def _build_phase_cell_list_ids(x, v1, v2, Lx, eta, eps1, eps2, Nx, radius):
    N = x.shape[0]
    v1_min, v1_max = _minmax(v1)
    v2_min, v2_max = _minmax(v2)
    ...
    for i in range(head.shape[0]):
        head[i] = -1
    nxt = np.empty(N, dtype=np.int64)
    ix_id = np.empty(N, dtype=np.int64)
    iv1_id = np.empty(N, dtype=np.int64)
    iv2_id = np.empty(N, dtype=np.int64)

    inv_eta = 1.0 / eta
    inv_eps1 = 1.0 / eps1
    inv_eps2 = 1.0 / eps2
    for p in range(N):
        ix = int(np.floor(x[p] * inv_eta))
        if ix < 0 or ix >= Nx:
            ix = ix % Nx
        iv1 = int(np.floor((v1[p] - v1_origin) * inv_eps1))
        iv2 = int(np.floor((v2[p] - v2_origin) * inv_eps2))
        if iv1 < 0:
            iv1 = 0
        elif iv1 >= ncv1:
            iv1 = ncv1 - 1
    ...
    return head, nxt, v1_origin, v2_origin, ncv1, ncv2, ix_id, iv1_id, iv2_id
...
@njit(cache=True)
def _build_x_batch_cell_list_ids(x, batch_ids, Lx, eta, Nx, R_eff):
    N = x.shape[0]
    head = np.empty(Nx * R_eff, dtype=np.int64)
    for i in range(head.shape[0]):
        head[i] = -1
...
@njit(parallel=True, fastmath=True, cache=True)
def _compute_ft_grad_pass_hat(x, v1, v2, w, head, nxt, ix_id, iv1_id, iv2_id, Lx, eta, eps1, eps2,
                              Nx, radius, v1_origin, v2_origin, ncv1, ncv2):
    N = x.shape[0]
    ft = np.empty(N, dtype=np.float64)
    g1 = np.empty(N, dtype=np.float64)
    g2 = np.empty(N, dtype=np.float64)
    ...
    inv_eps1 = 1.0 / eps1
    inv_eps2 = 1.0 / eps2
    inv_eps12 = inv_eps1 * inv_eps2
    grad1_scale = inv_eps1 * inv_eps1 * inv_eps2
    grad2_scale = inv_eps1 * inv_eps2 * inv_eps2
            ...
            for di1 in range(-rv1, rv1 + 1):
                ...
                for di2 in range(-rv2, rv2 + 1):
                    ...
                    while q != -1:
                        ...
                        bx = _hat_value(dx * inv_eta)
                        if bx != 0.0:
                            dv1 = v1p - v1[q]
                            dv2 = v2p - v2[q]
                            b1, db1 = _hat_value_derivative(dv1 * inv_eps1)
                            if b1 != 0.0 or db1 != 0.0:
                                b2, db2 = _hat_value_derivative(dv2 * inv_eps2)
                                if b2 != 0.0 or db2 != 0.0:
                                    wx = bx * inv_eta
                                    wqwx = w[q] * wx
                                    s0 += wqwx * (b1 * b2 * inv_eps12)
                                    s1 += wqwx * (db1 * b2 * grad1_scale)
                                    s2 += wqwx * (b1 * db2 * grad2_scale)
                        q = nxt[q]
        ft[p] = s0
        g1[p] = s1
        g2[p] = s2
...
@njit(parallel=True, fastmath=True, cache=True)
def _compute_F_second_pass_hat(x, v1, v2, w_over_ft, ft, grad1, grad2, head, nxt,
                               ix_id, iv1_id, iv2_id, Lx, eta, eps1, eps2, Nx, radius,
                               v1_origin, v2_origin, ncv1, ncv2):
    N = x.shape[0]
    F1 = np.empty(N, dtype=np.float64)
    F2 = np.empty(N, dtype=np.float64)
                        ...
                        if bx != 0.0:
                            ...
                            if b1 != 0.0 or db1 != 0.0:
                                ...
                                if b2 != 0.0 or db2 != 0.0:
                                    wx = bx * inv_eta
                                    coef = w_over_ft[q] * wx
                                    s1 += coef * (db1 * b2 * grad1_scale)
                                    s2 += coef * (b1 * db2 * grad2_scale)
                        q = nxt[q]
        inv_ftp = 1.0 / ft[p]
        F1[p] = grad1[p] * inv_ftp + s1
        F2[p] = grad2[p] * inv_ftp + s2
    return F1, F2


@njit(parallel=True, fastmath=True, cache=True)
def _compute_pair_accel_xbatch_hat_gamm2(xr, vr1, vr2, Fr1m, Fr2m, batch_r, ixr,
                                         xs, vs1, vs2, ws, Fs1m, Fs2m,
                                         head_src, nxt_src,
                                         Lx, eta, Nx, radius, R_eff,
                                         batch_scale, Bcoef, same_species):
    Nr = xr.shape[0]
    a1 = np.empty(Nr, dtype=np.float64)
    a2 = np.empty(Nr, dtype=np.float64)
    rx = int(np.ceil(radius))
                ...
                if (not same_species) or (q != p):
                    ...
                    if bx != 0.0:
                        ...
                        if r2 > 0.0:
                            ...
                            bF1 = Fs1m[q] - F1p
                            bF2 = Fs2m[q] - F2p
                            wq = ws[q] * bx * inv_eta * batch_scale
                            s1 += wq * (A11 * bF1 + A12 * bF2)
                            s2 += wq * (A12 * bF1 + A22 * bF2)
                q = nxt_src[q]
        a1[p] = s1
...
@njit(parallel=True, fastmath=True, cache=True)
def _compute_pair_accel_xbatch_hat_gamma0(xr, vr1, vr2, Fr1m, Fr2m, batch_r, ixr,
                                         xs, vs1, vs2, ws, Fs1m, Fs2m,
                                         head_src, nxt_src,
                                         Lx, eta, Nx, radius, R_eff,
                                         batch_scale, Bcoef, same_species):
...
@njit(parallel=True, fastmath=True, cache=True)
def _compute_pair_accel_xbatch_hat_general(xr, vr1, vr2, Fr1m, Fr2m, batch_r, ixr,
                                           xs, vs1, vs2, ws, Fs1m, Fs2m,
                                           head_src, nxt_src,
                                           Lx, eta, Nx, radius, R_eff,
                                           batch_scale, gam, Bcoef, same_species):
...
def _compute_species_F(x, v1, v2, w, eps1, eps2, par):
    ...
    if par.spline_order == 1:
        head, nxt, v1_origin, v2_origin, ncv1, ncv2, ix_id, iv1_id, iv2_id = _build_phase_cell_list_ids(
            x, v1, v2, par.Lx, par.eta, eps1, eps2, par.Nx, radius
        )
        ft, grad1, grad2 = _compute_ft_grad_pass_hat(
            x, v1, v2, w, head, nxt, ix_id, iv1_id, iv2_id, par.Lx, par.eta, eps1, eps2,
            par.Nx, radius, v1_origin, v2_origin, ncv1, ncv2
        )
        w_over_ft = w / ft
        F1, F2 = _compute_F_second_pass_hat(
            x, v1, v2, w_over_ft, ft, grad1, grad2, head, nxt, ix_id, iv1_id, iv2_id,
            par.Lx, par.eta, eps1, eps2, par.Nx, radius,
            v1_origin, v2_origin, ncv1, ncv2
        )
    else:
...
def _pair_accel_xbatch_dispatch(xr, vr1, vr2, Fr1m, Fr2m, batch_r, ixr,
                                xs, vs1, vs2, ws, Fs1m, Fs2m,
                                head_src, nxt_src, par, R_eff, batch_scale,
                                Bcoef, same_species):
    if par.spline_order == 1:
        if par.gam == -2.0:
            return _compute_pair_accel_xbatch_hat_gamm2(
                xr, vr1, vr2, Fr1m, Fr2m, batch_r, ixr, xs, vs1, vs2, ws, Fs1m, Fs2m,
                head_src, nxt_src, par.Lx, par.eta, par.Nx, par.spline_radius,
                R_eff, batch_scale, Bcoef, same_species
            )
        if par.gam == 0.0:
            return _compute_pair_accel_xbatch_hat_gamma0(
                xr, vr1, vr2, Fr1m, Fr2m, batch_r, ixr, xs, vs1, vs2, ws, Fs1m, Fs2m,
                head_src, nxt_src, par.Lx, par.eta, par.Nx, par.spline_radius,
                R_eff, batch_scale, Bcoef, same_species
            )
        return _compute_pair_accel_xbatch_hat_general(
            xr, vr1, vr2, Fr1m, Fr2m, batch_r, ixr, xs, vs1, vs2, ws, Fs1m, Fs2m,
            head_src, nxt_src, par.Lx, par.eta, par.Nx, par.spline_radius,
            R_eff, batch_scale, par.gam, Bcoef, same_species
        )
    return _compute_pair_accel_xbatch(
        xr, vr1, vr2, Fr1m, Fr2m, batch_r, xs, vs1, vs2, ws, Fs1m, Fs2m,
        head_src, nxt_src, par.Lx, par.eta, par.Nx, par.spline_order, par.spline_radius,
        R_eff, batch_scale, par.gam, Bcoef, same_species
    )
...
def compute_collision_acceleration(x1, v11, v12, w1, x2, v21, v22, w2,
    ...
    if par.spline_order == 1:
        head1_xb, nxt1_xb, ix1_xb = _build_x_batch_cell_list_ids(x1, batch1, par.Lx, par.eta, par.Nx, R_eff)
        head2_xb, nxt2_xb, ix2_xb = _build_x_batch_cell_list_ids(x2, batch2, par.Lx, par.eta, par.Nx, R_eff)
    else:
    ...
    a11_self, a12_self = _pair_accel_xbatch_dispatch(
        x1, v11, v12, F11m, F12m, batch1, ix1_xb,
        x1, v11, v12, w1, F11m, F12m,
        head1_xb, nxt1_xb, par, R_eff, scale_same_1, C * par.B11, True
    )
    a11_cross, a12_cross = _pair_accel_xbatch_dispatch(
        x1, v11, v12, F11m, F12m, batch1, ix1_xb,
        x2, v21, v22, w2, F21m, F22m,
        head2_xb, nxt2_xb, par, R_eff, scale_cross, C * par.B21, False
    )

    # Species 2: self B22 and source species 1 B12.
    a21_self, a22_self = _pair_accel_xbatch_dispatch(
        x2, v21, v22, F21m, F22m, batch2, ix2_xb,
        x2, v21, v22, w2, F21m, F22m,
        head2_xb, nxt2_xb, par, R_eff, scale_same_2, C * par.B22, True
    )
    a21_cross, a22_cross = _pair_accel_xbatch_dispatch(
        x2, v21, v22, F21m, F22m, batch2, ix2_xb,
        x1, v11, v12, w1, F11m, F12m,
        head1_xb, nxt1_xb, par, R_eff, scale_cross, C * par.B12, False
    )
