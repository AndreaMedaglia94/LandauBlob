...
This package includes implementation-level speedups in the Numba collision and PIC field routines.  The default `spline_order=1` and `gam=-2` path now uses specialized kernels; higher-order splines still use the generic kernels.  See `OPTIMIZATION_NOTES.md` for details and benchmark numbers.

For best throughput on a multicore machine, set Numba's thread count before running, for example:
